 ...
 * object lists, the environment, the chunks — is what `mapFormat.js` describes
 ...
 */
/** @param {MapFile[]} list */
export function resetMaps(list) {
  for (const id of Object.keys(MAPS)) delete MAPS[id];
  for (const map of list) registerMap(map);
}

for (const map of GAME_MAPS) registerMap(map);

export const START_MAP = startMap;
...
import { isGenerated } from './chunks.js';
import { currentRun } from './generate.js';
