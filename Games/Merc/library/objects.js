/**
 * The things that stand on a map.
 *
 * Found rather than listed. Every one of these is a folder under assets/ with
 * a object.json in it, so the folder *is* the record — copy the directory and
 * you have copied the object, pictures and all. A list here would be a
...
let found;
try {
  found = import.meta.glob('../assets/**/object.json', { eager: true, import: 'default' });
} catch {
  found = {};
}

export const PROPS = recordsFrom(found, 'object.json');
