/**
 * Arrangements of objects, placed as one.
 *
 * Found rather than listed. Every one of these is a folder under assets/ with
 * a prefab.json in it, so the folder *is* the record — copy the directory and
 * you have copied the prefab, pictures and all. A list here would be a
...
let found;
try {
  found = import.meta.glob('../assets/**/prefab.json', { eager: true, import: 'default' });
} catch {
  found = {};
}

export const PREFABS = recordsFrom(found, 'prefab.json');
